...
//			List<TransactionDAO> transactionDAOs=transactions.stream().map(this:: getTransactionDAO).collect(Collectors.toList());
//		userDAO.setTransactions(transactionDAOs);
//		userDAO.setTotalUsers(userRepo.count());
//		List<User> users= userRepo.findAll();
//		Double totalBalance=0.0;
////		for(User userData: users) {
////			totalBalance= totalBalance+userData.getAccount().getAccountBalance().doubleValue();
////		}
...
//		//mapToDouble(user->user.getAccount().getAccountBalance().doubleValue().su)//forEach(user-> user.getAccount().getAccountBalance());
//		
//		}
//		else {
//		
...
//	}
	 ...
	 @Override
	public List<UserDAO> getAllUsers() {
		List<User> users = userRepo.findAllByOrderByFirstNameAsc();
		return users.stream().map(this::transformUser)
				.collect(Collectors.toList());
	}
